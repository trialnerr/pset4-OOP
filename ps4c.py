    ...
    def apply_transpose(self, transpose_dict):
        ...
        for char in message: 
            if char in transpose_dict:     
                cipher += transpose_dict.get(char)
            else: 
                cipher += char
        return cipher
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for vowels_permutation in get_permutations(VOWELS_LOWER):
            ...
            dict = self.build_transpose_dict(vowels_permutation)
            possible_message = self.apply_transpose(dict)
            
            for char in possible_message.split():
                if is_word(self.valid_words, char):
                    print("hey you")
                    number_of_valid_words +=1 
                    print(number_of_valid_words)
